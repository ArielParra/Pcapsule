...
void headerLooker(u_char *user, const struct pcap_pkthdr *pkthdr, const u_char *packetd_ptr)
{
	...
	case IPPROTO_TCP:
	{
        ...
        std::cout << "TCP | SPORT: " <<  src_port << " | DPORT: " << dst_port;
        break;
	}
	case IPPROTO_UDP:
	{
	...
	case IPPROTO_ICMP:
	{
   		struct icmphdr *icmp_header = (struct icmphdr *)packetd_ptr;
        int icmp_type = icmp_header->type;
        int icmp_code = icmp_header->code;
        std::cout << "ICMP | TYPE: " << icmp_type << " | CODE: " << icmp_code ;
        break;
	}
	default:
		std::cout << "---"; // UNKOWN
